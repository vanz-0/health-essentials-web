-- Retry: fix pg_policies column name
-- 1) Timestamp helper
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'feature_flags' AND policyname = 'Authenticated users can insert feature flags'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'feature_flags' AND policyname = 'Authenticated users can update feature flags'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'feature_flags' AND policyname = 'Authenticated users can delete feature flags'
  ) THEN
