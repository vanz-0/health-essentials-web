-- Feature Flags foundation for Bit-based rollout
-- 1) Timestamp helper (idempotent)
...
-- 4) RLS policies
-- Public can read flags
create policy if not exists "Feature flags are viewable by everyone"
  on public.feature_flags
  for select
  using (true);

-- Only authenticated users can manage flags
create policy if not exists "Authenticated users can insert feature flags"
  on public.feature_flags
  for insert
  to authenticated
  with check (true);

create policy if not exists "Authenticated users can update feature flags"
  on public.feature_flags
  for update
  to authenticated
  using (true)
...
create policy if not exists "Authenticated users can delete feature flags"
  on public.feature_flags
  for delete
  to authenticated
  using (true);
