-- Fixing migration with idempotent policy creation via DO blocks
-- 1) Timestamp helper
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$create policy "Feature flags are viewable by everyone"
  ...
  IF NOT EXISTS (
    ...
    EXECUTE $$create policy "Authenticated users can insert feature flags"
      on public.feature_flags for insert to authenticated with check (true)$$;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE $$create policy "Authenticated users can update feature flags"
      on public.feature_flags for update to authenticated using (true) with check (true)$$;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE $$create policy "Authenticated users can delete feature flags"
      on public.feature_flags for delete to authenticated using (true)$$;
  END IF;
END$$;
